<script lang="ts">
	...
	async function handleCreateRoom() {
		...
		try {
			...
			modal?.close();
			await fetchRooms();
			newRoom = { name: '', capacity: 5, type: 'basic', pricePerHour: 100000 };
		} catch (error) {
					...
					{:else}
						{#each rooms as room}
							<tr class="hover">
								...
								<td>
									<div class={`badge badge-sm rounded-md font-bold uppercase ${room.type === 'vip' ? 'badge-secondary' : 'badge-ghost'}`}>
										{room.type}
									</div>
								</td>
								<td class="text-center font-medium">{room.capacity} Khách</td>
								<td class="text-right font-mono font-bold text-primary">
									{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(room.pricePerHour)}
								</td>
								<td class="text-right gap-2">
									<button class="btn btn-xs btn-ghost btn-square text-info hover:bg-info/10">
										<Icon icon="solar:pen-2-line-duotone" class="text-lg"/>
									</button>
									<button class="btn btn-xs btn-ghost btn-square text-error hover:bg-error/10">
			...
			<div class="grid grid-cols-2 gap-4">
				<label class="form-control w-full">
					...
					<select bind:value={newRoom.type} class="select select-bordered w-full rounded-md font-medium uppercase tracking-wider text-sm">
						<option value="basic">Basic (Thường)</option>
